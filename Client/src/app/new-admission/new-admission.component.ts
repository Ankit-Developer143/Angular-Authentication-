...
export class NewAdmissionComponent implements OnInit {
  ...
  total:new FormControl('',[Validators.required, Validators.minLength(3)]),
 ...
 //file:new FormControl({ value: '', disabled: this.disabled })
  selectedFile: File | undefined;
  constructor(private cs:CommonService,private snackbar:MatSnackBar,private router:Router) { 
  ...
  submit(){
    ...
    this.cs.addDetails(this.myForm.value).subscribe(d =>{
      console.log(d);
      this.snackbar.open("item add succefully", '', {
        duration: 3000
      });
      this.clearMethod();
